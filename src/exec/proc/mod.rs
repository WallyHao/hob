...
pub(crate) fn setenv(state: &mut State, request: &SetEnv) -> Result<Value, Failure> {
    session(state, request.session)?.setenv(&request.name, &request.value);
    Ok(Value::Null)
}

/// Remove one override.
pub(crate) fn unset(state: &mut State, request: &Unset) -> Result<Value, Failure> {
    session(state, request.session)?.unset(&request.name);
    Ok(Value::Null)
}

/// Move the session.
pub(crate) fn chdir(state: &mut State, request: &Chdir) -> Result<Value, Failure> {
    session(state, request.session)?.chdir(&request.path)?;
    Ok(Value::Null)
}

/// Replace the profile.
pub(crate) fn setup(state: &mut State, request: &Setup) -> Result<Value, Failure> {
    session(state, request.session)?.setup(&request.text);
    Ok(Value::Null)
}

/// Report where the session is now.
pub(crate) fn state_of(state: &State, request: &Handle) -> Result<Value, Failure> {
...
fn session_ref(state: &State, id: u64) -> Result<&Session, Failure> {
    state.procs.get(&id).ok_or_else(|| unknown(id))
}

fn session(state: &mut State, id: u64) -> Result<&mut Session, Failure> {
